...
const toUniqueSortedStrings = (rows, key) => {
...
function OrderEntryFormAutocomplete() {
  ...
  const handleSave = useCallback(async () => {
    if (!formData.article || !formData.ry_number || !formData.client) {
      setSnackbar({
        open: true,
        message: 'Vui lÃ²ng nháº­p Article, ÄÆ¡n HÃ ng vÃ  KhÃ¡ch hÃ ng.',
        severity: 'warning'
      });
      return;
    }
    ...
    try {
      ...
      setSnackbar({ open: true, message: 'LÆ°u Ä‘Æ¡n hÃ ng thÃ nh cÃ´ng.', severity: 'success' });
      handleReset();
    } catch (err) {
      setSnackbar({ open: true, message: `LÆ°u Ä‘Æ¡n hÃ ng tháº¥t báº¡i: ${err.message}`, severity: 'error' });
    } finally {
      setSaving(false);
    }
      ...
      <Paper elevation={0} sx={getEntryPaperSx}>
        <Box>
          <Typography variant="h5" sx={{ fontWeight: 700, color: '#0f172a', mb: 1 }}>
            Nháº­p ThÃ´ng Tin ÄÆ¡n HÃ ng Má»›i
          </Typography>
        </Box>
        ...
        <LocalizationProvider dateAdapter={AdapterDayjs}>
          ...
          >
            ...
            <Autocomplete
              ...
              onInputChange={(_, newInputValue) => updateField('article', newInputValue)}
              renderInput={(params) => <TextField {...params} fullWidth label="Article" />}
              sx={getFieldInputSx(formData.article)}
            />
            ...
            <Autocomplete
              ...
              onInputChange={(_, newInputValue) => updateField('ry_number', newInputValue)}
              renderInput={(params) => (
                <TextField
                  {...params}
                  fullWidth
              ...
              sx={getFieldInputSx(formData.ry_number)}
            />
            ...
            <Autocomplete
              ...
              onInputChange={(_, newInputValue) => updateField('delivery_round', newInputValue)}
              renderInput={(params) => <TextField {...params} fullWidth label="Äá»£t Giao HÃ ng" />}
              sx={getFieldInputSx(formData.delivery_round)}
            />

            <DatePicker
              label="NgÃ y Xuáº¥t Cáº£ng"
              format="DD/MM/YYYY"
              value={formData.CRD}
...
export default OrderEntryFormAutocomplete;
